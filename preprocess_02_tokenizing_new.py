...
try:
    for category in CATEGORIES:
        CSV_ORIGIN = f"./data_raw/cleaned_book_info_{category}.csv"
        CSV_RESULT_NO_SPACE = f"./data_token/book_token_{category}.csv"
        ...
        for idx in df.index:
            ...
            sentence = ' '.join(keywords_no_space)
            print(sentence)

            sentences_no_space.append(sentence)
            codes.append(idx)

        df_result = df[df.index.isin(codes)].copy()
        df_result['info'] = sentences_no_space

        df_result.to_csv(CSV_RESULT_NO_SPACE)
